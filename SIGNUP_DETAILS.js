...
async function finalSubmit() {
    ...
    if (!displayName) return showNotify("Display name is required. Cannot be empty.");
    if (!userId) return showNotify("Invalid User ID. Cannot be empty.");
    if (!checkPasswordStrength(password)) return showNotify("Password too weak. Must include uppercase, lowercase, and numbers (min 7 chars).");
    if (password !== confirmPass) return showNotify("Passwords do not match. Please verify your password again.");
    if (!birthMonth) return showNotify("Please select your birth month.");
    if (!birthDay) return showNotify("Please select your birth day.");
    if (!birthYear) return showNotify("Please select your birth year.");
    if (ageNum < 13) return showNotify("Minimum age requirement is 13 years old. Registration not allowed.");
